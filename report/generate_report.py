...
def build_report(output_path):
    ...
    story.append(Paragraph(
        "Option 3 failed because importNetworkFromPyTorch generates 30+ custom layers "
        ...
        "insufficient; the base class itself is incompatible. ShuffleNet V2's channel_shuffle "
        "(view+transpose+contiguous) and chunk operations have no built-in MATLAB layer "
        "equivalents. Note: Option 4 (ONNX) works because its custom layers use a different "
        "base class, and R2026a's GenerateCoderFiles=true creates a codegen-compatible +coder/ "
        "package. MATLAB inference works perfectly (6.98 ms, bit-identical to Option 4), so "
