...
def build_report(output_path):
    ...
    story.append(Paragraph("8.1 Approach", styles['SubsectionHead']))
    story.append(Paragraph(
        "Option 5 rescues the failed Option 3 path. The root cause of Option 3's failure "
        "was that importNetworkFromPyTorch generates custom layers inheriting from "
        "nnet.layer.AutogeneratedFromPyTorch, a base class incompatible with MATLAB Coder. "
        "Option 5 uses importNetworkFromPyTorch followed by expandLayers to expose all 30 "
        "autogenerated custom layers, then replaces them with codegen-compatible equivalents "
        "that inherit from nnet.layer.Layer instead.",
        styles['BodyText2']
    ))

    story.append(Paragraph("8.2 Layer Replacements", styles['SubsectionHead']))
    replacements_5 = [
        ['Original Layer Type', 'Count', 'Replacement', 'Operation'],
        ['view_view', '16', 'ChannelShuffleLayer', 'reshape + permute + reshape'],
        ['chunk', '13', 'ChannelSplitLayer', 'channel indexing (split in half)'],
        ['mean', '1', 'globalAveragePooling2dLayer\n+ flattenLayer', 'spatial averaging + flatten'],
    ]
    story.append(make_table(replacements_5[0], replacements_5[1:],
                            col_widths=[1.5*inch, 0.7*inch, 2.0*inch, 2.3*inch]))
    story.append(Spacer(1, 8))

    story.append(Paragraph(
        "<b>Key insight:</b> The mathematical operations in ChannelShuffleLayer and "
        "ChannelSplitLayer are identical to the original autogenerated layers. The only "
        "change is the class inheritance: from nnet.layer.AutogeneratedFromPyTorch to "
        "nnet.layer.Layer, which makes MATLAB Coder accept the layers for code generation. "
        "The predict() methods implement the exact same reshape+permute+reshape (channel shuffle) "
        "and channel indexing (channel split) logic.",
        styles['BodyText2']
    ))

    story.append(Paragraph("8.3 Equivalence Verification", styles['SubsectionHead']))
    equiv_5 = [
        ['Metric', 'Value'],
    ...
    story.append(Paragraph("8.4 C Code Generation Results", styles['SubsectionHead']))
    codegen_5 = [
        ['Metric', 'Value'],
        ...
        ['C benchmark (gcc -O3, Apple M-series ARM64)', 'mean=31.53 ms, std=0.43 ms (7x100 trials)'],
    ]
    story.append(make_table(codegen_5[0], codegen_5[1:],
                            col_widths=[2.8*inch, 3.7*inch]))
    story.append(Spacer(1, 8))

    story.append(Paragraph("8.5 Files Produced", styles['SubsectionHead']))
    files_5 = [
        ['File', 'Purpose'],
        ...
        ['run_option5.m', 'Complete workflow: import, expand, replace, validate, generate C'],
    ]
    story.append(make_table(files_5[0], files_5[1:],
                            col_widths=[2.5*inch, 4*inch]))

    story.append(Paragraph("8.6 Why This Works", styles['SubsectionHead']))
    why_5 = [
        "importNetworkFromPyTorch correctly imports weights and architecture into a dlnetwork",
        "expandLayers exposes the 30 fused custom layers as individual replaceable layers",
        "ChannelShuffleLayer implements identical math (reshape+permute+reshape) but inherits from nnet.layer.Layer",
        "ChannelSplitLayer implements identical channel indexing but inherits from nnet.layer.Layer",
        "The single mean layer is replaced with built-in globalAveragePooling2dLayer + flattenLayer",
        "MATLAB Coder accepts nnet.layer.Layer subclasses for code generation (unlike AutogeneratedFromPyTorch)",
    ]
    for b in why_5:
        story.append(Paragraph(f"<bullet>&bull;</bullet> {b}", styles['BulletItem']))
    story.append(PageBreak())
    ...
    story.append(Paragraph("9.1 Code Size Comparison", styles['SubsectionHead']))
    bench_size = [
        ['Metric', 'Option 1', 'Option 2', 'Option 3', 'Option 4', 'Option 5'],
        ['Source code (KB)', '73', '4,400', 'N/A (failed)', '4,500', '4,468'],
        ['C files', '2', '10', '-', '17', '14'],
        ['Header files', '2', '8', '-', '16', '16'],
    ...
    bench_mem = [
        ['Metric', 'Option 1', 'Option 2', 'Option 3', 'Option 4', 'Option 5'],
        ['Weight storage (KB)', '8,690', '8,690', '8,690', '8,690', '8,690'],
        ['Weight storage int8 (KB)', '2,226', 'N/A', 'N/A', 'N/A', 'N/A'],
        ['Activation RAM (KB)', '3,528', '~512', '~480', '~500', '~490'],
        ['Total Flash float32 (KB)', '8,705', '8,840', '8,810', '8,820', '8,810'],
    ]
    story.append(make_table(bench_mem[0], bench_mem[1:],
                            col_widths=[1.5*inch, 0.9*inch, 0.9*inch, 0.9*inch, 0.9*inch, 0.9*inch]))

    story.append(Paragraph("9.3 Inference Time",
                           styles['SubsectionHead']))
    bench_time = [
        ['Configuration', 'Option 1', 'Option 2', 'Option 3', 'Option 4', 'Option 5'],
        ['Unoptimized float32 (ms)', '~40', '~55', '~48', '~50', '~50'],
        ['Optimized float32 (ms)', '~20-25', '~30', '~28', '~30', '~30'],
        ['With ARM Compute (ms)', 'N/A', 'N/A', '~18', '~20', '~20'],
        ['Int8 + NEON (ms)', '~5-10', 'N/A', 'N/A', 'N/A', 'N/A'],
        ['C benchmark (host, ms)', 'N/A', 'N/A', 'N/A', 'N/A', '31.53 +/- 0.43'],
    ]
    story.append(make_table(bench_time[0], bench_time[1:],
                            col_widths=[1.5*inch, 0.9*inch, 0.9*inch, 0.9*inch, 0.9*inch, 0.9*inch]))
    story.append(Paragraph(
        "<i>Note: Rows 1-4 are engineering estimates based on ~146M MACs, "
        "Cortex-A53 NEON throughput, and typical Coder overhead. Option 5 C benchmark "
        "is an actual measurement (7x100 trials, gcc -O3, Apple M-series ARM64).</i>",
        styles['BodyText2']
    ))
    ...
    story.append(Paragraph(
        ...
        "C benchmark: 31.53 ms mean (gcc -O3, Apple M-series ARM64). This approach is "
