...
def build_report(output_path):
    ...
    story.append(Paragraph("8.1 Why Option 3 Fails (Root Cause Analysis)", styles['SubsectionHead']))
    story.append(Paragraph(
        "When importNetworkFromPyTorch encounters a PyTorch operation with no direct MATLAB "
        "equivalent layer, it creates a custom layer class inheriting from "
        "<b>nnet.layer.AutogeneratedFromPyTorch</b>. This base class works fine for MATLAB-only "
        "inference, but MATLAB Coder does not recognize it as a valid layer type for code generation. "
        "Crucially, adding %#codegen pragmas to these layers is insufficient because the Coder "
        "rejection happens at the class hierarchy level, not at the individual method level.",
        styles['BodyText2']
    ))
    story.append(Paragraph(
        "ShuffleNet V2 has three operations that trigger this behavior: (1) <b>channel shuffle</b> "
        "(PyTorch: view + transpose + contiguous), which produces 16 view_view_* layers; "
        "(2) <b>channel split</b> (torch.chunk), which produces 13 chunk_* layers; and "
        "(3) <b>global mean pooling</b> (torch.mean over spatial dims), which produces 1 mean_* layer. "
        "All 30 layers inherit from AutogeneratedFromPyTorch, blocking all code generation.",
        styles['BodyText2']
    ))

    story.append(Paragraph("8.2 The Insight: Same Math, Different Base Class", styles['SubsectionHead']))
    story.append(Paragraph(
        "The key insight is that the mathematical operations inside these 30 layers are simple: "
        "reshape, permute, indexing, and averaging. All of these are operations that MATLAB Coder "
        "fully supports. The <i>only</i> problem is the class hierarchy. By reimplementing the "
        "same predict() logic in custom layers that inherit from <b>nnet.layer.Layer</b> (which "
        "Coder supports), the entire Option 3 path becomes viable for code generation.",
        styles['BodyText2']
    ))
    ...
    why_5 = [
        ...
        "The rebuilt network is numerically equivalent (max error 4.77e-06) and achieves competitive C performance (31.53 ms, only 7.8% slower than Option 2)",
