...
static void avgPooling3525373489998439990(const float *inputTensor,
                                          float *outputTensor);

static void
c_avgPoolingKernel3525373489998(const float *inputBufferPtr,
                                float *outputBufferPtr, int poolOutputWidthIdx,
                                int c_startIdxWithPaddingOffsetInpu);

static void
c_maxPoolingKernel1220848366947(const float *inputBufferPtr,
                                float *outputBufferPtr, int poolOutputWidthIdx,
                                int c_startIdxWithPaddingOffsetInpu);

static void maxPooling12208483669476755913(const float *inputTensor,
                                           float *outputTensor);
...
#pragma omp parallel for num_threads(omp_get_max_threads())
  ...
  for (c_fusedOutputWidthAndChannelBlo = 0;
       c_fusedOutputWidthAndChannelBlo < 256;
       c_fusedOutputWidthAndChannelBlo++) {
    c_avgPoolingKernel3525373489998(
        &inputTensor[c_fusedOutputWidthAndChannelBlo * 196],
        &outputTensor[c_fusedOutputWidthAndChannelBlo << 2], 0, 0);
  }
}
 ...
 * Return Type  : void
 */
static void c_avgPoolingKernel3525373489998(const float *inputBufferPtr,
                                            float *outputBufferPtr,
                                            int poolOutputWidthIdx,
                                            int c_startIdxWithPaddingOffsetInpu)
{
  ...
  (void)poolOutputWidthIdx;
  c_idxToStrideInputBufferAlongWi = c_startIdxWithPaddingOffsetInpu;
  areOutputRegistersInitialized = false;
  for (poolWidthIdx = 0; poolWidthIdx < 7; poolWidthIdx++) {
    for (poolHeightIdx = 0; poolHeightIdx < 7; poolHeightIdx++) {
      ...
      if (!areOutputRegistersInitialized) {
        ...
        opReg_opHeightIdx_0 =
            vaddq_f32(ipReg_opHeightIdx_0, opReg_opHeightIdx_0);
      }
    }
    c_idxToStrideInputBufferAlongWi += 28;
  }
  opReg_opHeightIdx_0 =
      vmulq_f32(vdupq_n_f32(0.020408163F), opReg_opHeightIdx_0);
  vst1q_f32(&outputBufferPtr[0], opReg_opHeightIdx_0);
}
 ...
 * Return Type  : void
 */
static void c_maxPoolingKernel1220848366947(const float *inputBufferPtr,
                                            float *outputBufferPtr,
                                            int poolOutputWidthIdx,
                                            int c_startIdxWithPaddingOffsetInpu)
{
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
  ...
  for (c_fusedOutputWidthAndChannelBlo = 0;
    ...
    c_maxPoolingKernel1220848366947(
 ...
 * Return Type  : void
 */
void b_pool(const float X[50176], float Z[1024])
{
  avgPooling3525373489998439990(&X[0], &Z[0]);
}
...
{
  maxPooling12208483669476755913(&X[0], &Z[0]);
}
