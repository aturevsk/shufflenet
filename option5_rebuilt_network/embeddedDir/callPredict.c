...
static void c_matrixMultiply123701924125020(int M, int K, int N, int blockSizeM,
...
static void macroKernel12370192412502020982(int M, int K, int N, const float *A,
                                            int LDA, const float *B, int LDB,
                                            float *C, int LDC);

static void microKernel5230823758904451676(int K, const float *A, int LDA,
                                           const float *B, int LDB, float *C,
                                           int LDC);

static void microKernel8586148407642015204(int K, const float *A, int LDA,
                                           const float *B, int LDB, float *C,
                                           int LDC);
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(i, b_i)
      ...
      for (i0 = 1; i0 <= i0_ub_tmp; i0++) {
        ...
        macroKernel12370192412502020982(b_i, K2, N2, &A[i + M * k], M, ptrB, K,
                                        &C[i + M * b_j1], M);
      }
    }
  }
                                            ...
                                            float *C, int LDC)
{
  ...
  while (j < N) {
    ...
    idxA = 0;
    while (i <= M - 7) {
      microKernel8586148407642015204(K, &A[idxA], LDA, &B[idxB], LDB, &C[idxC],
                                     LDC);
      idxA += 7;
      idxC += 7;
      i += 7;
    ...
    while (i <= M - 1) {
      microKernel5230823758904451676(K, &A[idxA], LDA, &B[idxB], LDB, &C[idxC],
                                     LDC);
      idxA++;
      idxC++;
      i++;
...
{
  static cell_wrap_19 outT_f11[2];
  static cell_wrap_24 outT_f85[2];
  static const float fv[1000] = {
  ...
  static float b_X[301056];
  static float outT_f6_0_f1[181888];
  static float b_tempData[90944];
  static float d_tempData[90944];
  static float tempData[90944];
  static float Z[75264];
  static float X[50176];
  static float c_tempData[45472];
  static float outT_f7_0_f1[45472];
  static float outT_f8_0_f1[45472];
  static float outT_f29_0_f1[22736];
  static float outT_f30_0_f1[22736];
  static float outT_f33_0_f1[22736];
  static float outT_f4_0_f1[18816];
  static boolean_T bufferInitialized;
  float outT_f78_0_f1[11368];
  float outT_f79_0_f1[11368];
  float outT_f82_0_f1[11368];
  float b_Z[1024];
  int i;
  if (!bufferInitialized) {
    readDnnConstants_real32_T(&t0_Weights[0],
                              "./embeddedDir//largeDnnConstants_9031181.bin",
                              1024000);
  }
  bufferInitialized = true;
  conv2dDirectOptimizedColMajor(inputsT_0_f1, b_X);
  c_iPoolingForColumnMajorWithOpe(b_X, Z);
  b_conv2dDirectOptimizedColMajor(Z, outT_f4_0_f1);
  c_conv2dDirectOptimizedColMajor(outT_f4_0_f1, c_tempData);
  d_conv2dDirectOptimizedColMajor(Z, outT_f6_0_f1);
  e_conv2dDirectOptimizedColMajor(outT_f6_0_f1, outT_f7_0_f1);
  f_conv2dDirectOptimizedColMajor(outT_f7_0_f1, outT_f8_0_f1);
  ...
  memcpy(&tempData[0], &c_tempData[0], 45472U * sizeof(float));
  memcpy(&tempData[45472], &outT_f8_0_f1[0], 45472U * sizeof(float));
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  dlarray_reshape(tempData, b_tempData);
  /* predict  Forward pass: split channels in half. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  memcpy(&outT_f11[0].f1[0], &b_tempData[0], 45472U * sizeof(float));
  g_conv2dDirectOptimizedColMajor(&b_tempData[45472], c_tempData);
  h_conv2dDirectOptimizedColMajor(c_tempData, outT_f7_0_f1);
  i_conv2dDirectOptimizedColMajor(outT_f7_0_f1, c_tempData);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  /*  Permute: swap G and CperG dims -> [H, W, G, CperG, N] */
  memcpy(&tempData[0], &outT_f11[0].f1[0], 45472U * sizeof(float));
  memcpy(&tempData[45472], &c_tempData[0], 45472U * sizeof(float));
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  dlarray_reshape(tempData, b_tempData);
  /* predict  Forward pass: split channels in half. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  ...
  /*    Z2 is [H, W, C/2, N] (second half) */
  memcpy(&outT_f11[0].f1[0], &b_tempData[0], 45472U * sizeof(float));
  j_conv2dDirectOptimizedColMajor(&b_tempData[45472], c_tempData);
  k_conv2dDirectOptimizedColMajor(c_tempData, outT_f7_0_f1);
  l_conv2dDirectOptimizedColMajor(outT_f7_0_f1, c_tempData);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  /*    Z2 is [H, W, C/2, N] (second half) */
  memcpy(&outT_f11[0].f1[0], &b_tempData[0], 45472U * sizeof(float));
  m_conv2dDirectOptimizedColMajor(&b_tempData[45472], c_tempData);
  n_conv2dDirectOptimizedColMajor(c_tempData, outT_f7_0_f1);
  o_conv2dDirectOptimizedColMajor(outT_f7_0_f1, c_tempData);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  memcpy(&tempData[45472], &c_tempData[0], 45472U * sizeof(float));
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  dlarray_reshape(tempData, d_tempData);
  p_conv2dDirectOptimizedColMajor(d_tempData, outT_f29_0_f1);
  q_conv2dDirectOptimizedColMajor(outT_f29_0_f1, outT_f30_0_f1);
  r_conv2dDirectOptimizedColMajor(d_tempData, b_tempData);
  s_conv2dDirectOptimizedColMajor(b_tempData, outT_f29_0_f1);
  t_conv2dDirectOptimizedColMajor(outT_f29_0_f1, outT_f33_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  /*  Permute: swap G and CperG dims -> [H, W, G, CperG, N] */
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  cat(outT_f30_0_f1, outT_f33_0_f1, outT_f8_0_f1);
  b_dlarray_reshape(outT_f8_0_f1, outT_f7_0_f1);
  /* predict  Forward pass: split channels in half. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  u_conv2dDirectOptimizedColMajor(&outT_f7_0_f1[22736], outT_f29_0_f1);
  v_conv2dDirectOptimizedColMajor(outT_f29_0_f1, outT_f33_0_f1);
  w_conv2dDirectOptimizedColMajor(outT_f33_0_f1, outT_f29_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  /*  Permute: swap G and CperG dims -> [H, W, G, CperG, N] */
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  cat(&outT_f7_0_f1[0], outT_f29_0_f1, outT_f8_0_f1);
  b_dlarray_reshape(outT_f8_0_f1, c_tempData);
  /* predict  Forward pass: split channels in half. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  x_conv2dDirectOptimizedColMajor(&c_tempData[22736], outT_f29_0_f1);
  y_conv2dDirectOptimizedColMajor(outT_f29_0_f1, outT_f33_0_f1);
  ab_conv2dDirectOptimizedColMajo(outT_f33_0_f1, outT_f29_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  /*  Permute: swap G and CperG dims -> [H, W, G, CperG, N] */
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  cat(&c_tempData[0], outT_f29_0_f1, outT_f8_0_f1);
  b_dlarray_reshape(outT_f8_0_f1, outT_f7_0_f1);
  /* predict  Forward pass: split channels in half. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  bb_conv2dDirectOptimizedColMajo(&outT_f7_0_f1[22736], outT_f29_0_f1);
  cb_conv2dDirectOptimizedColMajo(outT_f29_0_f1, outT_f33_0_f1);
  db_conv2dDirectOptimizedColMajo(outT_f33_0_f1, outT_f29_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  eb_conv2dDirectOptimizedColMajo(&c_tempData[22736], outT_f29_0_f1);
  fb_conv2dDirectOptimizedColMajo(outT_f29_0_f1, outT_f33_0_f1);
  gb_conv2dDirectOptimizedColMajo(outT_f33_0_f1, outT_f29_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  hb_conv2dDirectOptimizedColMajo(&outT_f7_0_f1[22736], outT_f29_0_f1);
  ib_conv2dDirectOptimizedColMajo(outT_f29_0_f1, outT_f33_0_f1);
  jb_conv2dDirectOptimizedColMajo(outT_f33_0_f1, outT_f29_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  kb_conv2dDirectOptimizedColMajo(&c_tempData[22736], outT_f29_0_f1);
  lb_conv2dDirectOptimizedColMajo(outT_f29_0_f1, outT_f33_0_f1);
  mb_conv2dDirectOptimizedColMajo(outT_f33_0_f1, outT_f29_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  nb_conv2dDirectOptimizedColMajo(&outT_f7_0_f1[22736], outT_f29_0_f1);
  ob_conv2dDirectOptimizedColMajo(outT_f29_0_f1, outT_f33_0_f1);
  pb_conv2dDirectOptimizedColMajo(outT_f33_0_f1, outT_f29_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  cat(&outT_f7_0_f1[0], outT_f29_0_f1, outT_f8_0_f1);
  b_dlarray_reshape(outT_f8_0_f1, c_tempData);
  qb_conv2dDirectOptimizedColMajo(c_tempData, outT_f78_0_f1);
  rb_conv2dDirectOptimizedColMajo(outT_f78_0_f1, outT_f79_0_f1);
  sb_conv2dDirectOptimizedColMajo(c_tempData, outT_f7_0_f1);
  tb_conv2dDirectOptimizedColMajo(outT_f7_0_f1, outT_f78_0_f1);
  ub_conv2dDirectOptimizedColMajo(outT_f78_0_f1, outT_f82_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  /*  Permute: swap G and CperG dims -> [H, W, G, CperG, N] */
  memcpy(&outT_f29_0_f1[0], &outT_f79_0_f1[0], 11368U * sizeof(float));
  memcpy(&outT_f29_0_f1[11368], &outT_f82_0_f1[0], 11368U * sizeof(float));
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  c_dlarray_reshape(outT_f29_0_f1, outT_f30_0_f1);
  /* predict  Forward pass: split channels in half. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Z1 is [H, W, C/2, N] (first half) */
  /*    Z2 is [H, W, C/2, N] (second half) */
  memcpy(&outT_f85[0].f1[0], &outT_f30_0_f1[0], 11368U * sizeof(float));
  vb_conv2dDirectOptimizedColMajo(&outT_f30_0_f1[11368], outT_f78_0_f1);
  wb_conv2dDirectOptimizedColMajo(outT_f78_0_f1, outT_f82_0_f1);
  xb_conv2dDirectOptimizedColMajo(outT_f82_0_f1, outT_f78_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  /*  Permute: swap G and CperG dims -> [H, W, G, CperG, N] */
  memcpy(&outT_f29_0_f1[0], &outT_f85[0].f1[0], 11368U * sizeof(float));
  memcpy(&outT_f29_0_f1[11368], &outT_f78_0_f1[0], 11368U * sizeof(float));
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  c_dlarray_reshape(outT_f29_0_f1, outT_f30_0_f1);
  /* predict  Forward pass: split channels in half. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  ...
  /*    Z2 is [H, W, C/2, N] (second half) */
  memcpy(&outT_f85[0].f1[0], &outT_f30_0_f1[0], 11368U * sizeof(float));
  yb_conv2dDirectOptimizedColMajo(&outT_f30_0_f1[11368], outT_f78_0_f1);
  ac_conv2dDirectOptimizedColMajo(outT_f78_0_f1, outT_f82_0_f1);
  bc_conv2dDirectOptimizedColMajo(outT_f82_0_f1, outT_f78_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  /*    Z2 is [H, W, C/2, N] (second half) */
  memcpy(&outT_f85[0].f1[0], &outT_f30_0_f1[0], 11368U * sizeof(float));
  cc_conv2dDirectOptimizedColMajo(&outT_f30_0_f1[11368], outT_f78_0_f1);
  dc_conv2dDirectOptimizedColMajo(outT_f78_0_f1, outT_f82_0_f1);
  ec_conv2dDirectOptimizedColMajo(outT_f82_0_f1, outT_f78_0_f1);
  /* predict  Forward pass: channel shuffle. */
  /*    X is [H, W, C, N] (unformatted inside dlnetwork). */
  /*    Output Z is [H, W, C, N] with channels interleaved. */
  /*  Reshape: [H, W, C, N] -> [H, W, CperG, G, N] */
  ...
  /*  Flatten back: [H, W, G, CperG, N] -> [H, W, C, N] */
  c_dlarray_reshape(outT_f29_0_f1, outT_f30_0_f1);
  fc_conv2dDirectOptimizedColMajo(outT_f30_0_f1, X);
  d_iPoolingForColumnMajorWithOpe(X, b_Z);
  c_matrixMultiply123701924125020(1000, 1024, 1, 128, 128, 128, &t0_Weights[0],
                                  &b_Z[0], &outputs_0_f1[0]);
  for (i = 0; i < 1000; i++) {
    outputs_0_f1[i] += fv[i];
  }
}
